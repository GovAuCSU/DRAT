...
func (rs *GithubRepositoryScore) Score(ctx context.Context) error {
	...
	if *rs.R.Owner.Type != "Organization" {
		rs.RiskNote = append(rs.RiskNote, fmt.Sprintf("[INFO] Repository is not managed under an organisation"))
	}
	...
	if len(contributors) < 3 {
		rs.RiskNote = append(rs.RiskNote, fmt.Sprintf("[MEDIUM] Size of collaborator for the repository is %d which is less than 3", len(contributors)))
	}

	if time.Since(rs.R.GetPushedAt().Time) > (time.Duration(24*365) * time.Hour) {
		rs.RiskNote = append(rs.RiskNote, fmt.Sprintf("[MEDIUM] Repository has not been updated for a year"))
	}

	if time.Since(rs.R.GetCreatedAt().Time) < (time.Duration(24*120) * time.Hour) {
		rs.RiskNote = append(rs.RiskNote, fmt.Sprintf("[MEDIUM] Repository is young and only been created for less than 120 days"))
	}
	...
	if len(forks) > 10 {
		rs.RiskNote = append(rs.RiskNote, fmt.Sprintf("[GOOD] Repository has been forked %d times", len(forks)))
	}

	if rs.R.GetStargazersCount() > 50 {
		rs.RiskNote = append(rs.RiskNote, fmt.Sprintf("[GOOD] Repository has been stared %d times", rs.R.GetStargazersCount()))
	}

	if rs.R.GetWatchersCount() > 50 {
		rs.RiskNote = append(rs.RiskNote, fmt.Sprintf("[GOOD] Repository is being watched by %d people", rs.R.GetWatchersCount()))
	}

	if *rs.R.HasWiki {
		rs.RiskNote = append(rs.RiskNote, fmt.Sprintf("[GOOD] Repository has a wiki"))
	}
