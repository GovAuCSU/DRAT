...
function drawGraph() {
    ...
    for (var id in graph.data) {
        ...
        if (obj.DependedOnBy != "") {
            var link = {
                source : graph.data[obj.DependedOnBy],
                target : obj
            };
    ...
    for (var id in graph.data) {
        ...
        var obj = graph.data[id];
        obj.type = "green";
        if (obj.DependenciesCrawlProblems !=null && obj.DependenciesCrawlProblems.length > 0){
            obj.type = "yellow";
        }
        if (obj.RiskNotes.RISK !=null && obj.RiskNotes.RISK.length > 0){
            obj.type = "red";
        }
        var    key = obj.type + ':' + (obj.group || '');
        var    cat = graph.categories[key];
        obj.categoryKey = key;
        if (!cat) {
...
function selectObject(obj, el) {
    // THIS IS WHERE WE PULL INFO ABOUT REPO 
    var node;
    if (el) {
        node = d3.select(el);
    } else {
    ...
    // This is where we specify what information being shown in the pull-up div 
    $('#docs').html(JSON.stringify(obj, null, 2));
...
// Dropping JSON file  into browser 
// https://stackoverflow.com/questions/8869403/drag-drop-json-into-chrome
...
var reader = new FileReader();
    ...
    temp = JSON.parse(this.result);
    // Massage our data to conform with what expected by d3js
    for (var i = 0; i < temp.length; i++) { 
        data.data[temp[i].ID] = temp[i];
    }
    console.log("Data is loaded, let's draw!");
    ...
    $(document).on('click', '.select-object', function() {
        var obj = graph.data[$(this).data('id')];
        if (obj) {
            selectObject(obj);
        }
        return false;
